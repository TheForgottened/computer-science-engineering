...
{
	SOCKET sockfd;
	int iResult, nbytes, cli_len;
	struct sockaddr_in serv_addr, cli_addr;
	char buffer[BUFFERSIZE], resposta[5];
	WSADATA wsaData;
	...
	while(1){
		...
		printf("%i> ", ntohs(cli_addr.sin_port));
		printf("%s\n", buffer);

		sprintf(resposta, "%i", strlen(buffer));

		if(sendto( sockfd , resposta , strlen(resposta) , 0 , (struct sockaddr*)&cli_addr , sizeof(cli_addr) ) == SOCKET_ERROR)
			Abort("SO nao conseguiu confirmar a rececao do datagram");

		printf("\n<SER1> Confirmacao enviada...\n\n",buffer);
