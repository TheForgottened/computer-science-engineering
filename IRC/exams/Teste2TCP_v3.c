...
int main(int argc,char *argv[]) {
	...
	while (1) {
		...
		if (contador >= 4) {
			// Questão 5 - defina o timeout usando a função select e as variáveis fd_accept e tempoEspera
			FD_ZERO(&fd_accept);
		...
		if (!ocorrenciaTimeout) {
			...
			// Questão 7 - Envie a mensagem de boas vindas ao cliente, definida na constante MSG_BOAS_VINDAS
			send(newSock, MSG_BOAS_VINDAS, strlen(MSG_BOAS_VINDAS), 0);

			// Questão 8 - Construa a mensagem com identificador do número do cliente e envie-a ao cliente
			sprintf(cliente_id_msg, "Cliente%d", contador);
...
void AtendeCliente(LPVOID param) {
	...
	// Questão 9 - Envie a todos os cliente ligados a mensagem de arranque de conversa (definida na constante MSG_ARRANQUE_CONVERSA)
...
Recebe um caracter em s1 e reenvia-o para o grupo.
Não reenvia para s1.
...
int difunde(SOCKET origem, SOCKET *grupo, int tam, int sock_number) {
	...
	// ------ QUESTAO 13 NÃO FAZ SENTIDO NENHUM, SE É STATIC NAO SE DEVIA INICIALIZAR SEMPRE
	...
	if ((result = recv(origem, &c, sizeof(char), 0)) == sizeof(char)) {
		   for (i = 0; i < tam; i++) {
			   if (origem != grupo[i]) {
					...
					if (n_msg[i] % 2 == 1)
						result = send(grupo[i], &c, sizeof(c), 0);

					// Questão 16 - teste se o número da mensagem a enviar é par e converta vogais minúsculas em maiúsculas e vogais maiúsculas em minúsculas, conforme pedido
					if (n_msg[i] % 2 == 0) {
					    if (c == 'E')
                            c = '3'; // converta-o para o repectivo número
                        else if (c == 'I')
                            c = '1'; // converta-o para o repectivo número
                        else if (c == 'O')
                            c = '0'; // converta-o para o repectivo número
                        else if (c == 'S')
                            c = '5'; // converta-o para o repectivo número
                        else if (c == 'T')
                            c = '7'; // converta-o para o repectivo número
...
Mostra a mensagem de erro associada ao ultimo erro dos Winsock e abandona com "exit status" a 1
