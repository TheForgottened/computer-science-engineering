...
int main( int argc , char *argv[] )
{
	...
	SOCKET s;
	double sentValue;
	int iResult, nbytes, size, SERV_UDP_PORT;
	struct sockaddr_in serv_addr;
	char response[MAX_RESPONSE], SERV_UDP_ADDR[4096];
	...
	sentValue = atoi(argv[1]);
	strcpy(SERV_UDP_ADDR, argv[2]);
	SERV_UDP_PORT = atoi(argv[3]);
	...
	serv_addr.sin_addr.s_addr = inet_addr(SERV_UDP_ADDR);
	...
	nbytes = recvfrom(s, response, sizeof(response), 0, (struct sockaddr *)&serv_addr, &size);
